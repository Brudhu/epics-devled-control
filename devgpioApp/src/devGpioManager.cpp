...
//------------------------------------------------------------------------------
void GpioManager::setValue( epicsUInt32 gpio, char *val ) {
  std::map< epicsUInt32, GPIO >::iterator it = _mgpio.find( gpio );

  std::stringstream filename;
  printf("Val: %d", val);
  filename << _gpiobase << gpio << "/trigger";
...
epicsUInt32 GpioManager::getValue( epicsUInt32 gpio ) {
  ...
  std::stringstream filename;
  filename << _gpiobase << gpio << "/trigger"; //"/value" // Leds have brightness, not value
