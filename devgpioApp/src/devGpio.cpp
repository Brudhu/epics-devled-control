...
//------------------------------------------------------------------------------
long devGpioControlInit( int after ) {
...
//! @return  In case of error return -1, otherwise return 0
//------------------------------------------------------------------------------
long devGpioControlInitRecord( dbCommon *prec, devGpio_rec_t* pconf ){
  ...
  try{
    ...
    //GpioManager::instance().exportPin( gpioID ); // Not exporting leds
  ...
  // Get current value of "trigger" and write it on VAL
  try{
    pinfo->value = GpioManager::instance().getValue( pinfo->gpio );
    //pinfo->strValue = GpioManager::instance().getValue( pinfo->gpio );
    printf("pinfo->value (%d): %d\n", pinfo->gpio, pinfo->value);
  } catch( GpioManagerError &e ) {
...
//------------------------------------------------------------------------------
long devGpioControlGetIoIntInfo( int cmd, dbCommon *prec, IOSCANPVT *ppvt ){
...
//! @return  ERROR in case of an error, otherwise OK
//------------------------------------------------------------------------------
long devGpioControlRead( devGpio_info_t *pinfo ){
  try{
    pinfo->value = GpioManager::instance().getValue( pinfo->gpio );
  } catch( GpioManagerError &e ) {
    strncpy( pinfo->errmsg, e.what(), 255 );
...
//! @return  ERROR in case of an error, otherwise OK
//------------------------------------------------------------------------------
long devGpioControlWrite( devGpio_info_t *pinfo ){
  try{
    GpioManager::instance().setValue( pinfo->gpio, pinfo->strValue );
  } catch( GpioManagerError &e ) {
    strncpy( pinfo->errmsg, e.what(), 255 );
    return ERROR;
  }
...
extern "C" {
  ...
  epicsExportRegistrar( devLedControlConstRegister );
}
